...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    const parsed = roles.map(r => ({
      ...
      alert_thresholds: r.alert_thresholds ? JSON.parse(r.alert_thresholds) : null,
...
router.post('/', requireAuth, requireRole('CEO'), async (req, res) => {
  try {
    const db = await getDb();
    const { name, facilities, metrics, kpis, alert_thresholds } = req.body;
    if (!name) return res.status(400).json({ error: 'Role name is required' });

    const result = db.prepare(
      'INSERT INTO roles (name, facilities, metrics, kpis, alert_thresholds, is_default) VALUES (?, ?, ?, ?, ?, 0)'
    ).run(
      name,
    ...
    res.status(201).json({ id: result.lastInsertRowid, name, facilities, metrics, kpis, alert_thresholds: alert_thresholds || {}, is_default: false });
  } catch (err) {
...
// PUT /api/roles/my-role — any authenticated user can save their own role's alert thresholds (and optionally metrics/kpis)
router.put('/my-role', requireAuth, async (req, res) => {
  try {
    const db = await getDb();
    const userRole = req.user.role; // the user's assigned role name
    const role = db.prepare('SELECT * FROM roles WHERE name = ?').get(userRole);
    if (!role) return res.status(404).json({ error: 'Role not found' });

    const { facilities, metrics, kpis, alert_thresholds } = req.body;
    const fields = [];
    const values = [];
    ...
    if (alert_thresholds !== undefined) { fields.push('alert_thresholds = ?'); values.push(JSON.stringify(alert_thresholds)); }
    ...
    res.json({
      ...
      alert_thresholds: updated.alert_thresholds ? JSON.parse(updated.alert_thresholds) : null,
